...
```bash
ssh root@damsure.nandakrishnan.in 'bash -lc "cd /root/DrySpot-Damsure-Calc-App && git pull --ff-only origin main && cd backend && npm ci --include=dev && npm run build && NODE_ENV=production npm run db:migrate && npm prune --omit=dev && pm2 restart damsure-api --update-env"'
```

## Migration Status

- Production sets `NODE_ENV=production`, so clean npm installs omit dev dependencies by default. The deploy command deliberately uses `npm ci --include=dev` so TypeScript and Sequelize CLI are available for the build and migration, passes `NODE_ENV=production` explicitly to the migration command, then prunes dev-only packages before restarting PM2. Do not use a faster command that assumes stale build tooling is already installed.
