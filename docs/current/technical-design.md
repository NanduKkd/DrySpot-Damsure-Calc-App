...
- Both new-server replacement paths query every non-deleted warranty by `client_id`, not only rows with `active_client_id` populated. This preserves replacements created by the old process during the migration-to-restart window.
