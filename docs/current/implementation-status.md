...
### Active delivery integration (2026-07-31)

The local `codex/product-roadmap` integration lane now includes APP-101 measurement validation, APP-102 measurement deletion confirmation, APP-103 Android permission minimization, APP-104 strict update-manifest parsing, APP-105 Unicode PDF fonts, APP-106 shared-device session hardening, APP-107 local update-publication tooling, APP-108 CLI-first user administration, APP-109 durable managed-file cleanup, APP-110 sync-safe permanent warranty deletion, APP-111 logical-version synchronization, and APP-112 sync status and recovery UX. APP-106, APP-107, APP-108, APP-110, APP-111, and APP-112 passed independent exact-tip verification; APP-107 remains unpublished and externally gated. APP-113's contract is frozen and its local implementation is active in task `019fb61a-e4ed-77d0-a0e4-72cab4c51ac6`, supported by read-only preflight task `019fb61b-1c9f-7782-9676-534fa1394aad`.
...
- Current manager-branch gate after APP-106 integration: backend lint exits with warnings only, all 24 suites / 119 tests pass, and the TypeScript build passes. Flutter passes 190 default tests with one intentional staging-only skip, the dedicated staging parser passes, and analysis reports no issues.
- APP-106's exact candidate `3d4b5ef` passed independent T2 verification with no findings at any severity. All six V1 stale-echo CAS cases, V1/V2 transactional rollback, cross-tenant identifier/tombstone isolation, stale API and multipart suppression, synchronous cache clearing, SQLite-only cursor ownership, stale media cleanup, and deferred protected-image fencing pass. A physical shared Android device with two real non-production accounts remains the external acceptance gate.
- Current manager-branch gate after APP-112 integration: backend lint exits with 141 existing warnings and no errors, all 25 suites / 128 tests pass, and the TypeScript build passes. Flutter passes 209 default tests with one intentional staging-only skip, the dedicated staging parser passes, and analysis reports no issues. The production debug APK is 161,449,527 bytes with SHA-256 `3341132f247712cde92811f7a1b2e8bae996a6254e3aab6dfb7903d494dc3368`.
- APP-112's exact candidate `b98e692` passed independent T2 verification with zero P0-P3 findings. Production-shaped V1 photo receipt completion, N+1/session/tenant fencing, ambiguous retry identity, path reuse, typed recovery state, required-update blocking, reconnect signaling, and repeatable Flutter plugin generation pass. Disposable PostgreSQL proof and physical reconnect/two-account device scenarios remain external acceptance gates.
