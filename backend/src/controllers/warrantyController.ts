...
export const uploadWarranty = async (req: AuthRequest, res: Response) => {
  ...
  if (typeof client_id !== 'string' || !client_id.trim()) return reject(400, 'client_id is required');
  const parsedStartDate = new Date(start_date);
  if (Number.isNaN(parsedStartDate.getTime())) return reject(400, 'start_date must be a valid date');
  ...
  try {
    const client = await Client.findByPk(client_id, { paranoid: false, attributes: ['id', 'franchiseeId', 'deletedAt'] });
    if (!client) return reject(404, 'Client not found. Please sync client data and try again');
    if (client.franchiseeId !== franchiseeId) return reject(403, 'Unauthorized: Client does not belong to your franchisee');
    ...
    const warranty = await sequelize.transaction(async (transaction) => {
      const active = await Warranty.findAll({
        where: { activeClientId: client.id },
        transaction,
    ...
    await Promise.all(replacedPdfs.map(({ pdfUrl, pdfFileName }) => removeStoredPdf(pdfUrl, pdfFileName)));
    ...
    if (error?.name === 'SequelizeUniqueConstraintError') {
      return res.status(409).json({ error: 'An active warranty already exists. Please confirm replacement and try again.' });
    }
...
export const getWarranties = async (req: AuthRequest, res: Response) => {
  ...
  return res.json(await Warranty.findAll({ where: { clientId: client.id }, order: [['createdAt', 'DESC']] }));
};

export const downloadWarranty = async (req: AuthRequest, res: Response) => {
  const warranty = await Warranty.findOne({ where: { id: req.params.id }, include: [{ model: Client, where: { franchiseeId: req.user?.franchiseeId } }] });
  if (!warranty) return res.status(404).json({ error: 'Warranty not found or unauthorized' });
  const filename = warranty.pdfFileName || path.basename(new URL(warranty.pdfUrl, 'http://localhost').pathname);
